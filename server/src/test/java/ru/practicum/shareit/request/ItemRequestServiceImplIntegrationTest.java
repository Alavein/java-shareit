...
class ItemRequestServiceImplIntegrationTest {
    ...
    void beforeEach() {
        ...
        User user = new User(null, "Имя", "mail@yandex.ru");
        userRepository.save(user);
        userId = user.getId();

        ItemRequest itemRequest = new ItemRequest(null, "Описание", user, created);
        itemRequest = itemRequestRepository.save(itemRequest);
        requestId = itemRequest.getId();

        item = new Item(1, "Предмет", "Описание", true, user, itemRequest);

        itemRequestDto = new ItemRequestDto(requestId, "Описание", created, List.of());

        itemDtoRequest = new ItemDtoRequest(1, "Предмет", "Описание", true, requestId);
    }
    ...
    void getById() {
        ...
        assertEquals("Описание", itemRequestDto1.getDescription());
